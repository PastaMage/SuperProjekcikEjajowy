"""
klasyfikator (pipeline) (2+ eksperymenty)
normalizacja danych
...
def _apply_base_style(fig: plt.Figure) -> None:
...
def plot_platform_usage(df: pd.DataFrame) -> None:
    ...
    bars = ax.bar(PLATFORM_LABELS, counts.values, color=PLATFORM_COLORS, width=0.55, zorder=3)
...
def plot_depression_balance(df: pd.DataFrame) -> None:
    ...
    bars = ax.barh(counts.index, counts.values, color=colors, height=0.45, zorder=3)
